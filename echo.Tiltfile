...
phasm_HOME_DIR = "%s/.phasmhome.echo" % WORK_DIR
...
BP_HOME_ENV = struct(key='BP_phasm_HOME', value=phasm_HOME_DIR)
...
local_resource(
  ...
  serve_cmd=" ".join([
    "pnpm -F @phasmhub/chat exec bp dev",
  ...
  resource_deps=['login-phasm', 'readiness', 'run-pushpin', 'create-conv-fid-store', 'create-user-fid-store'],
  labels=['run'],
...
local_resource(
  ...
  resource_deps=['login-phasm', 'run-chat-integration'],
  labels=['run'],
...
local_resource(
  ...
  cmd=" ".join([
    ...
    'pnpm run -F @phasm/chat test:e2e'
  ]),
  env={
    BP_HOME_ENV.key: BP_HOME_ENV.value,
    'BP_API_URL': API.bp_api_url,
