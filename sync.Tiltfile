...
local_resource(
  ...
  cmd="pnpm turbo -F @phasmhub/synchronizer build && pnpm -F @bp-bots/sinlin exec bp add -y --use-dev",
...
botId = read_json('./bots/sinlin/.phasm/project.cache.json')['devId']
cmd = "bp chat %s" % botId
print("\n".join([
  "",
  "#######################################################",
