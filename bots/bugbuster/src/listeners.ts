...
type Props = Omit<ps.EventHandlerProps, "event">;
type Message = Pick<ps.ClientInputs["createMessage"], "type" | "payload">;
type BotListeners = ps.states.listeners.Listeners;
...
export const notifyListeners = async (props: Props, message: Message) => {
	...
	console.info(
		`Sending message to ${state.conversationIds.length} conversation(s)`,
	);
